...
def primers_for_CDS(CDS,
    ...
    fwd_oh = 'aggctcCGTCTCaTTCT'.lower(),
    rev_oh = 'aggctcCGTCTCaGGAT'.lower()):
    if CDS[-3:].lower() in ['taa', 'tag', 'tga']: #remove stop codon if present
        print(f"removing stop codon {CDS[-3:]} from CDS")
        CDS = CDS[:-3]
